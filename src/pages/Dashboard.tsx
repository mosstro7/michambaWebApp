...
function ClienteDashboard({ orders }: { orders: any[] }) {
  const navigate = useNavigate();
...
function EspecialistaDashboard({ orders }: { orders: any[] }) {
  ...
  return (
    <div className="space-y-6 pt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {orders.filter(o => o.estado === OrderStatus.ABIERTO && (filter === 'ALL' || o.categoriaId === filter)).map((order) => (
          <OrderCard key={order.id} order={order} />
        ))}
      </div>
    </div>
      ...
      <Card className="hover:border-blue-500 transition-colors">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-bold text-gray-900 leading-tight">{category?.nombre}</h3>
              <p className="text-xs text-gray-500">{order.barrio}</p>
            </div>
          </div>
          <span className={cn("text-[10px] font-bold px-2 py-1 rounded-full uppercase", statusColors[order.estado as OrderStatus])}>
            {order.estado}
          </span>
        </div>
